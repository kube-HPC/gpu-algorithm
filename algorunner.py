...
class Algorunner:
    ...
    def _loadAlgorithm(self, options):
        try:
            ...
            os.chdir('{cwd}/{folderName}'.format(cwd=cwd,folderName=folderName))
            print('.{fileName}, package: {package}'.format(fileName=fileName,package=folderName))
            mod = importlib.import_module('.{fileName}'.format(fileName=fileName), package=folderName)
    ...
    def _start(self, options):
        try:
            ...
            if (method is None):
                raise Exception('unable to find method {start}'.format(**methods))
